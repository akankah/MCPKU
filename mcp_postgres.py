...
# Thread pool untuk blocking psycopg2 calls
_executor = None
...
def _get_conn_sync(connection_string: str = None):
    ...
    if connection_string:
        import psycopg2
        return psycopg2.connect(cs), True  # True = dedicated conn, harus di-close
    p = _get_pool()
    conn = p.getconn()
    # Hapus SELECT 1 overhead — psycopg2 pool sudah handle keepalive sendiri
    # Hanya reset jika conn dalam keadaan error
    try:
        if conn.closed:
...
DB_POOL_MIN (default 2), DB_POOL_MAX (default 10) control connection pool size.
""")


@mcp.tool(name="list_tables", description="List all tables in the database grouped by schema.")
async def list_tables(connection_string: str = "") -> str:
    ...
    def _sync():
        ...
        try:
            ...
            _put_conn_sync(conn, dedicated)

    try:
        rows = await asyncio.get_event_loop().run_in_executor(
            _get_executor(), lambda: _retry_sync(_sync)
        )
        if not rows:
...
@mcp.tool(name="query", description="Execute a read-only SQL query and return results as JSON.")
async def query(sql: str, connection_string: str = "", max_rows: int = 100) -> str:
    ...
    try:
        ...
        output = json.dumps(result_list, indent=2, default=str, ensure_ascii=False)
...
@mcp.tool(name="run_query", description="Alias for query — execute a read-only SQL query.")
...
@mcp.tool(name="describe_table", description="Show detailed column info for a table.")
...
@mcp.resource(uri="postgres://{table_name}/schema", name="Table Schema",
              description="JSON schema for a database table", mime_type="application/json")
async def get_table_schema(table_name: str) -> str:
    ...
    try:
        rows = await asyncio.get_event_loop().run_in_executor(_get_executor(), _sync)
        if not rows:
            return json.dumps({"error": f"table {schema}.{tbl} not found"})
        columns = [{"name": r[0], "type": r[1], "nullable": r[2] == "YES", "default": r[3]} for r in rows]
        return json.dumps({"table": f"{schema}.{tbl}", "columns": columns}, indent=2)
    except Exception as e:
