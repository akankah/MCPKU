...
@mcp.tool(
    name="redis_set",
    description="Set a key-value pair with optional TTL (seconds). TTL=0 means no expiry."
)
...
@mcp.tool(
    name="redis_get",
    description="Get value for a key. Returns null if key doesn't exist."
)
...
@mcp.tool(
    name="redis_cache",
    description="Cache-aware setter: sets key with default TTL=3600s. Override ttl as needed."
)
...
@mcp.tool(
    name="redis_keys",
    description="Search keys by glob pattern (e.g. 'session:*', 'memory:*', 'cache:*')."
)
...
@mcp.tool(
    name="redis_lpush",
    description="Push values to the head of a list (for memory collections)."
)
...
@mcp.tool(
    name="redis_lrange",
    description="Get range of elements from a list. start=0, end=-1 means all."
)
...
@mcp.tool(
    name="redis_sadd",
    description="Add members to a set (ensures uniqueness)."
)
...
@mcp.tool(
    name="redis_hset",
    description="Set fields in a hash (for structured data like entities with attributes)."
)
...
@mcp.tool(
    name="redis_ttl",
    description="Get remaining TTL for a key (seconds). -1 = no expiry, -2 = key not found."
)
...
@mcp.tool(
    name="redis_flushdb",
    description="Clear ALL keys in current Redis database. Use with extreme caution."
)
async def redis_flushdb() -> str:
    try:
        r = _r()
        r.flushdb()
        return _ok("database flushed")
    except Exception as e:
        return _err(str(e))

@mcp.tool(
    name="redis_info",
    description="Get Redis server info: version, memory usage, connected clients, keyspace."
)
async def redis_info() -> str:
    try:
        ...
        return json.dumps({
            ...
            "total_keys": sum(int(v.get("keys", 0) or 0) for v in info.get("keyspace", {}).values()),
            "os": info.get("os", ""),
